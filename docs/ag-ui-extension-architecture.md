...
1. **Browser core** — conversation, streaming, reconnect, shared state, and
   human/agent actions. One HTML/JS payload, comfortably below 1 MB.
...
the canvas adapter is 1.3 KB raw / 0.62 KB gzip. The optional renderer remains
about 74 KB of generated JavaScript plus 1.19 MB of WASM before compression.
...
**Repository agent harness landed 2026-07-24.** `agent-harness.json` is now the
checked-in routing graph for repository authorities, progressive guides,
resource roots, commands, extension mechanics, and proof vocabulary.
`dev/agent-harness/check.py` validates its routes and emits the current
environment as text or JSON. Same Page Studio's `project-walkthrough`
extension consumes the same graph and accepts generic, host-resolved source
anchors, so architecture, ADR, folder, code, and Govern walkthroughs do not
require hand-built subsystem pages. This is repository orientation and
projection, not package installation or a Phase 2 exit receipt.
...
- A future packaging step may bundle these files into one artifact, but the
  source/runtime contract remains modular; “single file” is distribution, not
  architecture.
