...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The semantic import (Visual Instinct: the MobileSAM oracle/agent segment
    // tools) is optional infrastructure, not a boot requirement for the map,
    // board, or cement. Its assets are never committed here: the infographics
    // are gitignored the same as the MobileSAM weights and the onnxruntime
    // runtime, and are fetched or generated by
    // tests/semantic-import/prepare-mobilesam.sh and `npm install`. A
    // present-but-wrong asset (size or digest mismatch) still refuses to
    // boot, exactly as before, because that is drift worth catching early.
    // A missing asset only disables the feature: the browser's own segment
    // worker already fails closed with a specific error
    // (static/extensions/atlas/segment-worker.js's fetchChecked / source
    // fetch) the moment a person tries to use it, so there is no silent
    // fallback either way, just a later and more specific failure than a
    // boot-time refusal would give.
    let mut semantic_import_available = true;
    for source in atlas_core::SEGMENT_SOURCES {
        ...
        semantic_import_available &=
            verify_pinned_asset_if_present(&static_dir.join(relative), None, source.sha256)?;
    }
    semantic_import_available &= verify_pinned_asset_if_present(
    ...
    if !semantic_import_available {
        tracing::warn!(
            "one or more Visual Instinct semantic-import assets are missing; the map, board, \
             and cement still work, but atlas_segment_oracle_import / atlas_segment_agent_import \
             will fail when the browser tries to fetch them. Run \
             examples/same-page-atlas/tests/semantic-import/prepare-mobilesam.sh and `npm install` \
             in examples/same-page-atlas to enable it."
        );
    }
...
/// Like [`verify_pinned_asset`], except a missing file is not a boot
/// failure: it returns `Ok(false)` so the caller can disable the feature that
/// asset serves instead of refusing to start. A present file that fails
/// verification (wrong size or digest) still returns `Err`, unchanged.
fn verify_pinned_asset_if_present(
    path: &std::path::Path,
    ...
    verify_pinned_asset(path, expected_bytes, expected_sha256)?;
    Ok(true)
}
